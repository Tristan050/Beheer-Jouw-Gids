	...
	<div class="flex min-h-screen">
		...
		<div class="flex min-h-screen flex-1 flex-col">
			<header class="border-b border-slate-200 bg-white/80 px-6 py-5 backdrop-blur">
				<h1 class="text-2xl font-semibold tracking-tight">Aandachtspunten beheren</h1>
			</header>
			...
			<main class="flex-1 space-y-6 px-6 py-6">
				...
				<nav class="flex items-center gap-2 text-sm text-slate-500" aria-label="Breadcrumb">
					...
					<span class="font-medium text-slate-700">Aandachtspunten</span>
				</nav>
						...
						<table class="min-w-full divide-y divide-slate-200" id="aandachtspuntTable" data-source-table="gids_aandachtspunt">
							<thead class="bg-slate-100 text-left text-xs font-semibold uppercase tracking-wide text-slate-500">
								<tr>
									...
									<th class="px-4 py-3">Scan_tekst</th>
									<th class="px-4 py-3">Advies_tekst</th>
									<th class="px-4 py-3">Acties</th>
								</tr>
							</thead>
							<tbody id="aandachtspuntTableBody" class="divide-y divide-slate-200 bg-white">
								<?php if (!empty($data['items'])): ?>
									<?php foreach ($data['items'] as $row): ?>
										<tr data-search="<?= htmlspecialchars((string) ($row['search'] ?? '')) ?>">
											...
											<td class="px-4 py-3 text-sm text-slate-700"><?= htmlspecialchars((string) ($row['scan_tekst'] ?? '')) ?></td>
											<td class="px-4 py-3 text-sm text-slate-700"><?= htmlspecialchars((string) ($row['advies_tekst'] ?? '')) ?></td>
											<td class="px-4 py-3">
												<div class="flex flex-wrap gap-2">
													<a href="<?= htmlspecialchars((string) ($row['edit_url'] ?? appUrl('aandachtspunt-edit'))) ?>" class="rounded-lg border border-slate-200 bg-white px-3 py-1.5 text-xs font-semibold text-slate-700 shadow-sm transition hover:border-slate-300 hover:bg-slate-50">Bewerken</a>
													<form method="post" action="<?= htmlspecialchars(appUrl('aandachtspunt-delete')) ?>" onsubmit="return confirm('Weet je zeker dat je dit aandachtspunt wilt verwijderen?');">
														<?= CSRF::token() ?>
								...
								<?php else: ?>
									<tr>
										<td colspan="7" class="px-4 py-6 text-center text-sm text-slate-500">Nog geen aandachtspunten gevonden.</td>
									</tr>
								<?php endif; ?>
							</tbody>
						</table>
