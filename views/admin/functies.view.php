	...
	<div class="flex min-h-screen">
		...
		<div class="flex min-h-screen flex-1 flex-col">
			<header class="border-b border-slate-200 bg-white/80 px-6 py-5 backdrop-blur">
				<h1 class="text-2xl font-semibold tracking-tight">Functies beheren</h1>
			</header>
			...
			<main class="flex-1 space-y-6 px-6 py-6">
				...
				<nav class="flex items-center gap-2 text-sm text-slate-500" aria-label="Breadcrumb">
					...
					<span class="font-medium text-slate-700">Functies</span>
				</nav>
				...
				<section class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
					...
					<div class="mt-4 flex flex-wrap items-center gap-3">
						<div class="relative min-w-[220px] flex-1">
							<input id="functieSearchInput" type="text" class="w-full rounded-lg border border-slate-200 bg-white px-3 py-2 pr-9 text-sm shadow-sm focus:border-[#A53714] focus:outline-none focus:ring-2 focus:ring-[#A53714]/20" placeholder="Zoek op functie of leefgebied..." />
							<span class="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400" aria-hidden="true">&#128269;</span>
						</div>
						<button type="button" class="rounded-lg border border-slate-200 bg-white px-4 py-2 text-sm font-semibold text-slate-700 shadow-sm transition hover:border-slate-300 hover:bg-slate-50" onclick="document.getElementById('functieSearchInput').value=''; filterFuncties();">Wissen</button>
					</div>

					<div class="mt-4 overflow-x-auto rounded-xl border border-slate-200">
						<table class="min-w-full divide-y divide-slate-200" id="functieTable" data-source-table="gids_functie">
							<thead class="bg-slate-100 text-left text-xs font-semibold uppercase tracking-wide text-slate-500">
								<tr>
									<th class="px-4 py-3">FunctieID</th>
									<th class="px-4 py-3">LeefgebiedID</th>
									<th class="px-4 py-3">Naam_functie</th>
									<th class="px-4 py-3">Beschrijving_functie</th>
									<th class="px-4 py-3">Sort_order</th>
									<th class="px-4 py-3">Acties</th>
								</tr>
							</thead>
							<tbody id="functieTableBody" class="divide-y divide-slate-200 bg-white">
										...
										<tr data-search="<?= htmlspecialchars((string) ($row['search'] ?? '')) ?>">
											...
											<td class="px-4 py-3">
												<div class="flex flex-wrap gap-2">
													<a href="<?= htmlspecialchars((string) ($row['edit_url'] ?? appUrl('functie-edit'))) ?>" class="rounded-lg border border-slate-200 bg-white px-3 py-1.5 text-xs font-semibold text-slate-700 shadow-sm transition hover:border-slate-300 hover:bg-slate-50">Bewerken</a>
													<form method="post" action="<?= htmlspecialchars(appUrl('functie-delete')) ?>" onsubmit="return confirm('Weet je zeker dat je deze functie wilt verwijderen?');">
														<?= CSRF::token() ?>
								...
								<?php else: ?>
									<tr>
										<td colspan="6" class="px-4 py-6 text-center text-sm text-slate-500">Nog geen functies gevonden.</td>
									</tr>
								<?php endif; ?>
							</tbody>
						</table>
